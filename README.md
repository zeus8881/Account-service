...
**`Spring Boot 3.x`** | **`Java 17/21`** | **`PostgreSQL (Docker)`** | **`SonarQube`** | **`HTTPS & RBAC`**
...
*   `POST /api/auth/changepass` — Змiна пароля `[Авторизовані користувачі]` *(Діє перевірка на TOP-100 слабких паролів)*
...
У проєкті рeaлізовано швидкі **зрізові інтеграційні тести (Slice Integration Tests)** для контролерів та веб-інфраструктури через `MockMvc` із підміною бізнес-шару через `@MockBean`. 
